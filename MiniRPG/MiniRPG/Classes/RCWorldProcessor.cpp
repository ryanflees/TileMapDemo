...
RCWorldProcessor::RCWorldProcessor():m_controllerLayer()
        ,m_player()
        ,m_tileMap()
        ,m_gameNode()
        ,m_delegate()
...
RCWorldProcessor::~RCWorldProcessor()
{
...
void RCWorldProcessor::updateControllerVector(RCControllerLayer* controllerLayer, cocos2d::CCPoint pressedVector,float delta)
{
    if (!m_player) {
        return;
    }
...
void RCWorldProcessor::setTileMapGameNode(cocos2d::CCNode *gameNode,cocos2d::CCTMXTiledMap* tileMap)
{
    m_gameNode = gameNode;
    m_tileMap = tileMap;
...
void RCWorldProcessor::setActorArray(cocos2d::CCArray *actorArray)
{
    m_actorArray = actorArray;
}

void RCWorldProcessor::adjustCamera(CCPoint moveOffset)
{
...
void RCWorldProcessor::setControllerLayerRef(RCControllerLayer* controllerLayer)
{
    if (m_controllerLayer) {
        m_controllerLayer->setDelegate(NULL);
    }
...
void RCWorldProcessor::setPlayerRef(RCActor *player)
{
    m_player = player;
    if (m_player) {
        m_playerAction = kActionStandDown;
...
void RCWorldProcessor::onEnter()
{
    CCNode::onEnter();
    scheduleUpdate();
}

void RCWorldProcessor::onExit()
{
    CCNode::onExit();
    unscheduleUpdate();
}

void RCWorldProcessor::update(float delta)
{
    if (!m_player) {
        return;
    }
...
CCSize RCWorldProcessor::getTilemapSize()
{
    if (!m_delegate) {
...
CCSize RCWorldProcessor::getTilemapSizeInPixels()
{
    if (!m_delegate) {
        return CCSizeMake(0, 0);
    }
...
CCPoint RCWorldProcessor::tilePosFromLocation(CCPoint location,CCTMXTiledMap* tileMap)
{
...
void RCWorldProcessor::checkObjectInteract(cocos2d::CCPoint point)
{
}

CCRect RCWorldProcessor::getRectFromObjectProperties(CCDictionary* dict,CCTMXTiledMap*tileMap)
{
    float x, y, width, height;
...
bool RCWorldProcessor::checkBarrierCollision(cocos2d::CCPoint position)
{
...
bool RCWorldProcessor::checkPositionInsideTileMap(cocos2d::CCPoint position)
{
    CCSize mapSize = getTilemapSizeInPixels();
    CCRect mapRect = CCRectMake(kPlayerOnMapEdgeSize.width
                                , kPlayerOnMapEdgeSize.height
...
CCPoint RCWorldProcessor::checkBarrierCollisionForOffset(cocos2d::CCPoint offset, RCActor* player)
{
...
void RCWorldProcessor::setActorPositionOnTile(RCActor *actor, cocos2d::CCDictionary *tileDict)
{
    float x, y, width, height;
