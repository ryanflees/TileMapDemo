...
void CCEGLViewProtocol::handleTouchesBegin(int num, int ids[], float xs[], float ys[], unsigned int hash[])
{
    CCSet set;
    for (int i = 0; i < num; ++i)
    {
        ...
        if (pIndex == NULL)
        {
			...
			pTouch->setTouchInfo(nUnusedIndex, (x - m_obViewPortRect.origin.x) / m_fScaleX, 
                                     (y - m_obViewPortRect.origin.y) / m_fScaleY, hashKey);
...
void CCEGLViewProtocol::handleTouchesMove(int num, int ids[], float xs[], float ys[], unsigned int hash[])
{
    CCSet set;
    for (int i = 0; i < num; ++i)
    {
...
void CCEGLViewProtocol::getSetOfTouchesEndOrCancel(CCSet& set, int num, int ids[], float xs[], float ys[], unsigned int hash[])
{
    for (int i = 0; i < num; ++i)
    {
        int id = ids[i];
...
void CCEGLViewProtocol::handleTouchesEnd(int num, int ids[], float xs[], float ys[], unsigned int hash[])
{
    CCSet set;
    getSetOfTouchesEndOrCancel(set, num, ids, xs, ys, hash);
    m_pDelegate->touchesEnded(&set, NULL);
}

void CCEGLViewProtocol::handleTouchesCancel(int num, int ids[], float xs[], float ys[], unsigned int hash[])
{
    CCSet set;
    getSetOfTouchesEndOrCancel(set, num, ids, xs, ys, hash);
